...
public class ImageGridAdapter extends BaseAdapter {
	...
	public ImageGridAdapter(Context context, boolean showCamera, int type) {
		mContext = context;
		this.type = type;
		mInflater = (LayoutInflater) context
				.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		...
		void bindData(final Image data) {
			...
			if (showSelectIndicator) {
				...
				if (mSelectedImages.contains(data)) {
					// 设置选中状态
					indicator
							.setImageResource(R.drawable.icon_xuanzhong_circle);
					mask.setVisibility(View.VISIBLE);
				} else {
					// 未选择
					indicator.setImageResource(R.drawable.photo_cancel);
			...
			if (mItemSize > 0) {
				if (type == 1) {
					...
					image.setImageBitmap(Util.getVideoThumbnail(data.path, 96,
							96, Images.Thumbnails.MICRO_KIND));
				} else {
					videoIcon.setVisibility(View.GONE);
					// 显示图片
					// BitmapUtils bitmapUtils = new BitmapUtils(mContext);
					// bitmapUtils.display(image, data.path);
					ImageLoader.getInstance().displayImage(
							"file://" + data.path, image,
