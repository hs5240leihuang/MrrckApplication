...
public final class BadgeUtil {
	 ...
	 * <p/>
	 * MIUI Sony huawei Samsung LG HTC Nova
	 * 
	 * 魅族flyme不支持角标
	 * android原生不支持角标
	 * 
	...
	public static void setBadgeCount(Context context, int count, int iconResId) {
		...
		LogUtil.e("hl", "Build.MANUFACTURER = " + Build.MANUFACTURER);
...
//		} 
			...
			LogUtil.e("hl", "Not Found Support Launcher");
		}
	}
	...
	private static void setBadgeOfHuawei(Context context, int count) {
		boolean isSupported = checkIsSupportedByVersion(context);
		LogUtil.e("hl", "huawei_isSupported_Badge="+isSupported);
		if (isSupported) {
			Bundle extra = new Bundle();
			extra.putString("package", context.getPackageName());
			extra.putString("class", AppInfoUtil.getLauncherClassName(context));
