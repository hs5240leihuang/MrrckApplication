 ...
 * 收藏简历
 */
public class CollectResumeFragment extends BaseFragment implements
		OnClickListener {
	...
	private int page;
	private List<ResumeCollectEntity> showlist = new ArrayList<ResumeCollectEntity>();
	private CommonAdapter<ResumeCollectEntity> showAdapter;
	...
	private void initPullListView() {
		...
		showAdapter = new CommonAdapter<ResumeCollectEntity>(getActivity(),
				R.layout.item_my_recruit, showlist) {

			@Override
			public void convert(ViewHolder viewHolder,
					final ResumeCollectEntity t) {
				...
				viewHolder.getView(R.id.tv_position).setVisibility(View.GONE);
...
//													+ t.getResumeId()));
	...
	private void GetData() {
		...
		req.setHeader(new ReqHead(AppConfig.BUSINESS_COMPANY_COLLECTION_LIST));
	 ...
	 * 删除简历
	 */
	private void deleteSelectResumes(String resumeCollectId) {
		ReqBase req = new ReqBase();
		Map<String, Object> map = new HashMap<String, Object>();
		if (!Tool.isEmpty(AppContext.getInstance().getUserInfo()
				.getCompanyEntity())) {
		...
		map.put("resumeCollectId", resumeCollectId);
		req.setHeader(new ReqHead(AppConfig.BUSINESS_COMPANY_DELETE_COLLECTION));
		req.setBody(JsonUtil.Map2JsonObj(map));
	...
	public <T> void onSuccess(int requestCode, T arg0) {
		...
		switch (requestCode) {
		case reqCodeOne:
			if (resp.getBody().get("resumeCollect").toString().length() > 2) {
				List<ResumeCollectEntity> listData = (List<ResumeCollectEntity>) JsonUtil
						.jsonToList(resp.getBody().get("resumeCollect")
								.toString(),
								new TypeToken<List<ResumeCollectEntity>>() {
								}.getType());
