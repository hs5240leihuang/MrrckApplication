...
import com.meiku.dev.bean.JobResumeSendEntity;
 ...
 * 收到简历
 */
public class ReceiveResumeFragment extends BaseFragment implements
		OnClickListener {
	...
	private int page;
	private List<JobResumeSendEntity> showlist = new ArrayList<JobResumeSendEntity>();
	private CommonAdapter<JobResumeSendEntity> showAdapter;
	private TextView tv_select;
	private boolean isSelectAll;
	private boolean isShowEdit;
	private String companyResumeDetailUrl;// 跳转简历详情url
	...
	protected BroadcastReceiver receiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if (BroadCastAction.ACTION_IM_REFRESH_COMPANY.equals(intent
					.getAction())) {
				downRefreshData();
			}else if (BroadCastAction.ACTION_PUBLIC_POSITION.equals(intent
					.getAction())) {
				downRefreshData();
			}
		}
	...
	private void initPullListView() {
		...
		showAdapter = new CommonAdapter<JobResumeSendEntity>(getActivity(),
				R.layout.item_my_recruit, showlist) {

			@Override
			public void convert(ViewHolder viewHolder,
					final JobResumeSendEntity t) {
				...
				if ("2".equals(t.getSendStatus())) {// 0:未查看 1:安排面试 2:已查看
					viewHolder.setText(R.id.tv_status, "已查看");
				} else if ("1".equals(t.getSendStatus())) {
					viewHolder.setText(R.id.tv_status, "安排面试");
				} else {
						...
						new OnClickListener() {
							...
							public void onClick(View arg0) {
								...
								if (!Tool.isEmpty(companyResumeDetailUrl)
									...
									if ("0".equals(t.getSendStatus())) {
										updateResumesLook(t.getId() + "");
									}
	...
	private void GetData() {
		...
		req.setHeader(new ReqHead(AppConfig.BUSINESS_EMPLOY_RESUME_LIST));
	 ...
	 * 删除简历
	 */
	private void deleteSelectResumes(String resumeSendId) {
		ReqBase req = new ReqBase();
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("userId", AppContext.getInstance().getUserInfo().getId());
		map.put("resumeSendId", resumeSendId);
		req.setHeader(new ReqHead(AppConfig.BUSINESS_EMPLOY_DELETE_RESUME));
		req.setBody(JsonUtil.Map2JsonObj(map));
	 ...
	 * 更新投递简历信息
	 */
	private void updateResumesLook(String resumeSendId) {
		ReqBase req = new ReqBase();
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("resumeSendId", resumeSendId);
		req.setHeader(new ReqHead(AppConfig.BUSINESS_UPDATE_RESUME_INFORMATION));
		req.setBody(JsonUtil.Map2JsonObj(map));
	...
	public <T> void onSuccess(int requestCode, T arg0) {
		...
		LogUtil.e("hl", "收到简历######" + resp.getBody().toString());
		switch (requestCode) {
		case reqCodeOne:
			if (resp.getBody().get("resumeSend").toString().length() > 2) {
				List<JobResumeSendEntity> listData = (List<JobResumeSendEntity>) JsonUtil
						.jsonToList(
								resp.getBody().get("resumeSend").toString(),
								new TypeToken<List<JobResumeSendEntity>>() {
								}.getType());
