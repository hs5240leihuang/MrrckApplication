...
public class TitleTabsLayout {
	private TranslateAnimation animation;
	private List<MkMenu> titletabList = new ArrayList<MkMenu>();
	...
	public TitleTabsLayout(Context context, List<MkMenu> titletabList,
	...
	private void init() {
		view = LayoutInflater.from(context).inflate(R.layout.view_titletabs,
				null, false);
		TabGroups = (LinearLayout) view.findViewById(R.id.TabGroups);
		selectBg = (LinearLayout) view.findViewById(R.id.selectBg);
		if (titletabList.size() > 0) {
			titleTabWidth = ScreenUtil.getWindowWidth(context)
					/ titletabList.size();
			selectBg.setLayoutParams(new RelativeLayout.LayoutParams(
					titleTabWidth, LayoutParams.FILL_PARENT));
	...
	public void SetSelecetedIndex(int index) {
		if (currentIndex != index) {
			setTitleTabTextClo(index, R.color.titletabsclo);
	...
	public void SetAllTitleClo(int selectedIndex) {
		for (int i = 0, size = TabGroups.getChildCount(); i < size; i++) {
			if (i == selectedIndex) {
				setTitleTabTextClo(i, R.color.white);
			} else {
				setTitleTabTextClo(i, R.color.titletabsclo);
			}
		}
	}
	...
	private void setTitleTabTextClo(int childIndex, int clo) {
		((TextView) TabGroups.getChildAt(childIndex)).setTextColor(context
				.getResources().getColor(clo));
	}
