...
public class MyShowTabsLayout {
	...
	public MyShowTabsLayout(Context context,
	...
	private void init() {
		view = LayoutInflater.from(context).inflate(R.layout.view_myshowtitletabs, null, false);
	...
	public void SetAllTitleClo(int selectedIndex) {
		for (int i = 0, size = TabGroups.getChildCount(); i < size; i++) {
			if (i == selectedIndex) {
				setTitleTabTextClo(i, R.color.mrrck_bg);
			} else {
	...
	private void setTitleTabTextClo(int childIndex, int clo) {
		((TextView) TabGroups.getChildAt(childIndex))
				.setTextColor(context.getResources().getColor(clo));
	}
