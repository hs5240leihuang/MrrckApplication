...
public class TeamDataCache {
	...
	public void buildCache() {
		final List<Team> teams = NIMClient.getService(TeamService.class)
				.queryTeamListBlock();
	...
	private List<TeamDataChangedObserver> teamObservers = new ArrayList<TeamDataChangedObserver>();
	private List<TeamMemberDataChangedObserver> memberObservers = new ArrayList<TeamMemberDataChangedObserver>();

	public void registerObservers(boolean register) {
		NIMClient.getService(TeamServiceObserver.class).observeTeamUpdate(
				teamUpdateObserver, register);
		NIMClient.getService(TeamServiceObserver.class).observeTeamRemove(
				teamRemoveObserver, register);
		NIMClient.getService(TeamServiceObserver.class).observeMemberUpdate(
				memberUpdateObserver, register);
		NIMClient.getService(TeamServiceObserver.class).observeMemberRemove(
				memberRemoveObserver, register);
	}
	...
	private Observer<List<Team>> teamUpdateObserver = new Observer<List<Team>>() {
		...
		public void onEvent(final List<Team> teams) {
			if (teams != null) {
				LogUtil.e("hl", "team update size:" + teams.size());
			}
			addOrUpdateTeam(teams);
	...
	public void registerTeamMemberDataChangedObserver(
			TeamMemberDataChangedObserver o) {
	...
	public void unregisterTeamMemberDataChangedObserver(
			TeamMemberDataChangedObserver o) {
	 ...
	 */
	public void fetchTeamById(final String teamId,
			final SimpleCallback<Team> callback) {
				...
				.setCallback(new RequestCallbackWrapper<Team>() {
					...
					public void onResult(int code, Team t, Throwable exception) {
						...
						if (code == ResponseCode.RES_SUCCESS) {
							...
							success = false;
							LogUtil.e("teamcache",
									"fetchTeamById failed, code=" + code);
						}

						if (exception != null) {
							success = false;
							LogUtil.e("teamcache",
									"fetchTeamById throw exception, e="
	...
	public Team getTeamById(String teamId) {
		...
		if (team == null) {
			team = NIMClient.getService(TeamService.class).queryTeamBlock(
					teamId);
			addOrUpdateTeam(team);
		}
	...
	public String getTeamName(String teamId) {
		...
		return team == null ? teamId : TextUtils.isEmpty(team.getName()) ? team
				.getId() : team.getName();
	}
	 ...
	 */
	public void fetchTeamMemberList(final String teamId,
			final SimpleCallback<List<TeamMember>> callback) {
		NIMClient.getService(TeamService.class).queryMemberList(teamId)
				.setCallback(new RequestCallbackWrapper<List<TeamMember>>() {
					@Override
					public void onResult(int code,
							final List<TeamMember> members, Throwable exception) {
						boolean success = true;
						if (code == ResponseCode.RES_SUCCESS) {
							...
							success = false;
							LogUtil.e("teamcache",
									"fetchTeamMemberList failed, code=" + code);
						}

						if (exception != null) {
							success = false;
							LogUtil.e("teamcache",
									"fetchTeamMemberList throw exception, e="
											+ exception.getMessage());
						}
	 ...
	 */
	public void fetchTeamMember(final String teamId, final String account,
			final SimpleCallback<TeamMember> callback) {
		NIMClient.getService(TeamService.class)
				.queryTeamMember(teamId, account)
				.setCallback(new RequestCallbackWrapper<TeamMember>() {
					@Override
					public void onResult(int code, TeamMember member,
							Throwable exception) {
						boolean success = true;
						if (code == ResponseCode.RES_SUCCESS) {
							addOrUpdateTeamMember(member);
							...
							success = false;
							LogUtil.e("teamcache",
									"fetchTeamMember failed, code=" + code);
						}

						if (exception != null) {
							success = false;
							LogUtil.e("teamcache",
									"fetchTeamMember throw exception, e="
											+ exception.getMessage());
						}
	...
	public TeamMember getTeamMember(String teamId, String account) {
		...
		if (!map.containsKey(account)) {
			TeamMember member = NIMClient.getService(TeamService.class)
	...
	public String getTeamMemberDisplayName(String tid, String account) {
		if (account.equals(AppContext.getInstance().getUserInfo()
	...
	/**
	 * 获取显示名称。用户本人也显示昵称 高级群：首先返回群昵称。没有群昵称，则返回备注名。没有设置备注名，则返回用户昵称。
	 * 讨论组：首先返回备注名。没有设置备注名，则返回用户昵称。
	 */
	public String getDisplayNameWithoutMe(String tid, String account) {
		IMYxUserInfo user = MsgDataBase.getInstance().getYXUserById(account);
		if (AppContext.getInstance().getFriendTalkIdMap().containsKey(account)) {
			Integer userId = AppContext.getInstance().getFriendTalkIdMap()
					.get(account);
			if (AppContext.getInstance().getRemarkMap().containsKey(userId)) {
				return AppContext.getInstance().getRemarkMap().get(userId);
			}
		} else if (user != null) {
