...
public class WatchVideoActivity extends BaseActivity implements Callback {
        ...
        public void run() {
            if (mediaPlayer != null && mediaPlayer.isPlaying()) {
                ...
                if (videoLength <= 0) {
                    ...
                    long seconds = DateTimeUtil.getSecondsByMilliseconds(leftTimes);
                    playTimeTextView.setText(DateTimeUtil.secToTime((int) seconds));
    ...
    private void showVideoInfo() {
        ...
        if (duration <= 0) {
            fileInfoTextView.setText("大小: " + FileHelper.formatFileSize(fileSize));
        } else {
            long seconds = DateTimeUtil.getSecondsByMilliseconds(duration);
            fileInfoTextView.setText("大小: " + FileHelper.formatFileSize(fileSize) + ",时长: "
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                downloadProgressText.setText(String.format(getString(R.string.download_progress_description), label,
                		FileHelper.formatFileSize(progress), FileHelper.formatFileSize(total)));
            }
        });
    }
