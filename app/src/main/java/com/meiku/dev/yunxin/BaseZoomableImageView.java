...
public abstract class BaseZoomableImageView extends View {
	...
	public void setImageBitmap(final Bitmap bitmap, final boolean fitScreen) {
		...
		if (Build.VERSION.SDK_INT >= MIN_SDK_ENABLE_LAYER_TYPE_HARDWARE) {
			if (bitmap != null
					&& (bitmap.getHeight() > SampleSizeUtil.getTextureSize() || bitmap
							.getWidth() > SampleSizeUtil.getTextureSize())) {
		...
		if (oldBitmap != null && oldBitmap != mBitmap
		...
		if (adjustLongImageEnable) {
			...
			if ((float) mBitmap.getHeight() / (float) mBitmap.getWidth() > MAX_IMAGE_RATIO_LARGE) {
				...
				scale = fw;
					...
					&& (float) mBitmap.getHeight() / (float) mBitmap.getWidth() > MAX_IMAGE_RATIO_WIDTH_LARGE_LANDSCAPE) {
				needAdjust = true;
				scale = fw;
			}
		}
	...
	protected boolean isScrollOver(float distanceX) {
		try {
			if (mDisplayMatrix != null) {
				...
				// mBitmap.getWidth() * getValue(mDisplayMatrix,
				// Matrix.MSCALE_X) 代表 当前图片显示宽度
				// width == mBitmap.getWidth() * getValue(mDisplayMatrix,
				// Matrix.MSCALE_X) 意味着图片右边离屏宽度 == 0，已经滑到最右边
