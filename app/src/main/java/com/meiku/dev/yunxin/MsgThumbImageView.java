...
public class MsgThumbImageView extends ImageView {
	...
	private static final DisplayImageOptions createImageOptions() {
		...
		return new DisplayImageOptions.Builder()
				.showImageOnLoading(defaultIcon).showImageOnFail(defaultIcon)
	...
	protected void onDraw(Canvas canvas) {
		if (mask != null) {
			...
			//
			{
				canvas.saveLayer(0, 0, width, height, paintMask,
	...
	public void loadAsPath(boolean isOriginal, final String path,
			final String tag, final int width, final int height,
			final int maskId) {
		...
		if (!hasLoaded) {
			...
			ImageLoader.getInstance().displayImage(uri,
					new NonViewAware(imageSize, ViewScaleType.CROP), options,
	...
	private void loadBmpAsync(final String imageUri, final String path,
			final ImageSize imageSize, final String tag) {
		NimSingleThreadExecutor.getInstance().execute(
				new NimSingleThreadExecutor.NimTask<Bitmap>() {
					@Override
					...
					public void onCompleted(Bitmap result) {
						if (result != null
								&& (getTag() != null && getTag().equals(tag) && !hasLoaded)) {
	...
	private Bitmap decodeBmpAndSave(String path, ImageSize imageSize,
			String imageUri) {
		Bitmap bitmap = BitmapDecoder.decodeSampled(path, imageSize.getWidth(),
				imageSize.getHeight());
		if (bitmap != null) {
			String memoryCacheKey = MemoryCacheUtils.generateKey(imageUri,
					imageSize);
			ImageLoader.getInstance().getMemoryCache()
					.put(memoryCacheKey, bitmap);
			try {
