 ...
 */
public class CaptureVideoActivity extends BaseActivity implements
	...
	private LinkedList<Point> backCameraSize = new LinkedList<Point>();

	private LinkedList<Point> frontCameraSize = new LinkedList<Point>();
	...
	private Runnable runnable = new Runnable() {
		...
		public void run() {
			...
			recordingTimeTextView.setText(DateTimeUtil.secToTime(invs));

			// 录制过程中红点闪烁效果
			if (invs % 2 == 0) {
				recordingState
						.setBackgroundResource(R.drawable.nim_record_start);
			} else {
				recordingState
						.setBackgroundResource(R.drawable.nim_record_video);
			}
			if (invs >= VIDEO_TIMES) {
				stopRecorder();
				sendVideo();
	...
	private void getVideoPreviewSize(boolean isFront) {
		...
		if (isCompatible(11)) {
			if (CamcorderProfile.hasProfile(cameraId,
				...
				LogUtil.e(TAG, (isFront ? "Back Camera" : "Front Camera")
						+ " no QUALITY_480P");
			}
	...
	private void setCamcorderProfile() {
		...
		if (profile != null) {
			...
			if (Build.MODEL.equalsIgnoreCase("MB525")
					|| Build.MODEL.equalsIgnoreCase("C8812")
	...
	public void updateRecordUI() {
		if (recording) {
			recordBtn
					.setBackgroundResource(R.drawable.nim_video_capture_stop_btn);
		} else {
			recordBtn
					.setBackgroundResource(R.drawable.nim_video_capture_start_btn);
		}
	}
	...
	private void startRecorder() {
		try {
			...
			Toast.makeText(this, R.string.start_camera_to_record_failed,
	...
	private void stopRecorder() {
		if (mediaRecorder != null) {
			try {
				...
				LogUtil.e(TAG, getString(R.string.stop_fail_maybe_stopped));
			}
			mediaRecorder.release();
			mediaRecorder = null;
		}
	...
	private void sendVideo() {
		...
		if (convertedFile.exists()) {
			...
			int kb = b / 1024;
			float mb = kb / 1024f;
			message += mb > 1 ? getString(R.string.capture_video_size_in_mb, mb)
					: getString(R.string.capture_video_size_in_kb, kb);
		...
		final CommonDialog commonDialog = new CommonDialog(
				CaptureVideoActivity.this, "提示", message, "确定");
		commonDialog
	...
	private void tooShortAlert() {
		final CommonDialog commonDialog = new CommonDialog(
				CaptureVideoActivity.this, "提示",
				getString(R.string.video_record_short), "确定");
		commonDialog
				.setClicklistener(new CommonDialog.ClickListenerInterface() {
					@Override
					public void doConfirm() {
	...
	private boolean initCamera() {
		try {
			...
			Toast.makeText(this, R.string.connect_vedio_device_fail,
					Toast.LENGTH_SHORT).show();
			return false;
		}
	...
	public int setCameraDisplayOrientation(Context context, int cameraId,
		...
		if (front) {
			...
			if ("Xiaomi_MI-ONE Plus".equalsIgnoreCase(Build.MANUFACTURER + "_"
					+ Build.MODEL)) {
				result = 90;
			}
		}
