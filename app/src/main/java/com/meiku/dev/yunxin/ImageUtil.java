...
public class ImageUtil {
	...
	public static Bitmap getDefaultBitmapWhenGetFail() {
        try {
            return getBitmapImmutableCopy(MrrckApplication.getInstance().getResources(), R.drawable.nim_image_download_failed);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static File getScaledImageFileWithMD5(File imageFile, String mimeType) {
        ...
        if (!isInvalidPictureFile(mimeType)) {
            LogUtil.e("ImageUtil", "is invalid picture file");
            return null;
        }

        String tempFilePath = getTempFilePath(FileHelper.getExtensionName(filePath));
    ...
    private static String getTempFilePath(String extension) {
    	return StorageUtil.getWritePath(
    			...
    			"temp_image_" + Util.get36UUID() + "." + extension,
    ...
    public static int[] getBoundWithLength(int maxSide, Object imageObject, boolean resizeToDefault) {
        ...
        if (String.class.isInstance(imageObject)) {
        	...
        	bound = BitmapDecoder.decodeBound(MrrckApplication.getInstance().getResources(), (Integer) imageObject);
        	width = bound[0];
        	height = bound[1];
    ...
    public static Bitmap getBitmapFromDrawableRes(int res) {
        try {
            return getBitmapImmutableCopy(MrrckApplication.getInstance().getResources(), res);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
