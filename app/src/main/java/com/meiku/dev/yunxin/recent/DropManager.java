...
public class DropManager {
	...
	static final int CIRCLE_RADIUS = ScreenUtil.dip2px(
			MrrckApplication.getInstance(), 8); // 10dip
	...
	private int[] explosionResIds = new int[] { R.drawable.explosion_one,
			R.drawable.explosion_two, R.drawable.explosion_three,
			R.drawable.explosion_four, R.drawable.explosion_five };

	// interface
	public void init(Context context, DropCover dropCover,
			DropCover.IDropCompletedListener listener) {
		...
		LogUtil.e(TAG, "init DropManager, statusBarHeight=" + statusBarHeight);
	}
	...
	public void destroy() {
		...
		LogUtil.e(TAG, "destroy DropManager");
	}
	...
	public Paint getCirclePaintWhite() {
		circlePaint = new Paint();
	...
	public TextPaint getTextPaint() {
		 ...
		 * 即这里ascent为负值，descent为正值。 比如ascent为-20，descent为5，那需要移动的距离就是20 - （20 +
		 * 5）/ 2
		 */
		textYOffset = -textFontMetrics.ascent
				- (-textFontMetrics.ascent + textFontMetrics.descent) / 2;

		return textPaint;
	}

	public TextPaint getTextPaintRed() {
		textPaint = new TextPaint();
		textPaint.setAntiAlias(true);
