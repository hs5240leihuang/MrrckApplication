...
public class MsgViewHolderAudio extends MsgViewHolderBase {
	...
	protected void onItemClick() {
		if (audioControl != null) {
			...
			audioControl.startPlayAudioDelay(CLICK_TO_PLAY_AUDIO_DELAY,
					message, onPlayListener);
	...
	private void layoutByDirection() {
		if (isReceivedMessage()) {
			...
			containerView.setPadding(ScreenUtil.dip2px(context, 15),
					ScreenUtil.dip2px(context, 8),
					ScreenUtil.dip2px(context, 10),
					ScreenUtil.dip2px(context, 8));
			animationView
					.setBackgroundResource(R.drawable.nim_audio_animation_list_left);
			...
			containerView.setPadding(ScreenUtil.dip2px(context, 10),
					ScreenUtil.dip2px(context, 8),
					ScreenUtil.dip2px(context, 15),
					ScreenUtil.dip2px(context, 8));
			animationView
					.setBackgroundResource(R.drawable.nim_audio_animation_list_right);
	...
	private void refreshStatus() {// 消息状态
		...
		if (status == MsgStatusEnum.sending
				|| attachStatus == AttachStatusEnum.transferring) {
		...
		// unread indicator
		if (isReceivedMessage() && attachStatus == AttachStatusEnum.transferred
	...
	private void controlPlaying() {
		final AudioAttachment msgAttachment = (AudioAttachment) message
				.getAttachment();
		...
		if (!isMessagePlaying(audioControl, message)) {
			if (audioControl.getAudioControlListener() != null
					&& audioControl.getAudioControlListener().equals(
							onPlayListener)) {
	...
	public static int getAudioMinEdge() {
		return (int) (0.1875 * ScreenUtil.SCREEN_WIDTH);
	}

	private void setAudioBubbleWidth(long milliseconds) {
		long seconds = DateTimeUtil.getSecondsByMilliseconds(milliseconds);

		int currentBubbleWidth = calculateBubbleWidth(seconds,
				MAX_AUDIO_TIME_SECOND);
	...
	private int calculateBubbleWidth(long seconds, int MAX_TIME) {
		...
		if (seconds <= 0) {
			...
			currentBubbleWidth = (int) ((maxAudioBubbleWidth - minAudioBubbleWidth)
					* (2.0 / Math.PI) * Math.atan(seconds / 10.0) + minAudioBubbleWidth);
		} else {
	...
	protected boolean isMessagePlaying(MessageAudioControl audioControl,
			IMMessage message) {
		if (audioControl.getPlayingAudio() != null
				&& audioControl.getPlayingAudio().isTheSame(message)) {
			return true;
		} else {
			return false;
		}
	...
	private void play() {
		if (animationView.getBackground() instanceof AnimationDrawable) {
			AnimationDrawable animation = (AnimationDrawable) animationView
					.getBackground();
			animation.start();
		}
	}
	...
	public void reclaim() {
		super.reclaim();
		if (audioControl.getAudioControlListener() != null
				&& audioControl.getAudioControlListener()
						.equals(onPlayListener)) {
			audioControl.changeAudioControlListener(null);
		}
	}
