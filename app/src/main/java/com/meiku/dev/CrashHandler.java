...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	private String saveCrashInfo2File(Throwable ex) {
		...
		sb.append(result);
		updateErrorMessage(sb.toString());//上传错误信息
		try {
	 ...
	 * @param errorMesssage
	 */
	public void updateErrorMessage(String errorMesssage) {
		ReqBase req = new ReqBase();
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("userId", AppContext.getInstance().getUserInfo().getId());
		map.put("devType", ConstantKey.DEV_TYPE);
		map.put("clientVersion", version);
		map.put("errorMsg", errorMesssage);
		req.setHeader(new ReqHead(AppConfig.BUSINESS_UPLOAD_ERRORMESSAGE));
		...
		StringJsonRequest request = new StringJsonRequest(AppConfig.DOMAIN
			...
			public void onResponse(String arg0) {
				...
				if (ConstantKey.REQ_SUCCESS.equals(head.getRetStatus())) {
					Log.e(TAG, "upload error message success");
				} else {
					Log.e(TAG, "upload error message filed");
				}
			}
